...
in_em = []
out_em = []
with open('100D_vanilla_GloVe.txt', 'r') as f:
    f.readline()
    l = f.readline()
    c = 0
    while l:
...
for w in words:
    ...
    if w in poincare_words:
        ...
        x = np.random.uniform(-0.001, 0.001, 100)
        embedds.append(np.random.uniform(-0.001, 0.001, 100))
        out_em.append(x)
...
plt.show()
pickle.dump(embedds, open('100d_snli_embedds_vanilla_glove', 'wb'))
